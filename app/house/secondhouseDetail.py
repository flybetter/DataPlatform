...
class secondhouseDetail:
    ...
    def get_avg_price_histogram(self):
        self.fiter_df['avg_price_range'] = pd.cut(self.fiter_df['PRICE_AVG'], bins=20, right=False, duplicates='drop')
    ...
    def get_area_histogram(self):
        try:
            self.df['PIC_AREA'] = pd.cut(self.df['PIC_AREA'], bins=10, right=False, duplicates='drop')
    ...
    def get_toilet_pie(self):
        try:
            temp = pd.DataFrame(
                {'Percentage': self.df.groupby("PIC_WEI").size() / len(
                    self.df['PIC_WEI'].dropna())}).reset_index().rename(
